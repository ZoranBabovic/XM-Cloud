...
if ($c -eq $true) {
    ...
    Get-ChildItem -Path (Join-Path $PSScriptRoot "data\cm\logs") -Exclude ".gitkeep" -Recurse | Remove-Item -Force -Recurse -Verbose
    Get-ChildItem -Path (Join-Path $PSScriptRoot "data\cd\logs") -Exclude ".gitkeep" -Recurse | Remove-Item -Force -Recurse -Verbose
}
